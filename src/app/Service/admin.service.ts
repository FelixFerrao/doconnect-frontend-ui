...
import { Admin } from '../MyComponents/admin/Model/admin';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { Answer } from '../MyComponents/user/Model/answer';
import { User } from '../MyComponents/login/Model/user';
...
export class AdminService {
  ...
  sendAdminData(admin: Admin) {
    ...
    this.admin.active = admin.active;
  }
  ...
  approveQuestion(questionId: number): Observable<Question> {
    return this.http.put<Question>(
      this.url + '/approveQuestion/' + questionId,
      this.question
    );
  }
  ...
  approveAnswer(answerId: number): Observable<Answer> {
    return this.http.put<Answer>(
      this.url + '/approveAnswer/' + answerId,
      Answer
    );
  }
